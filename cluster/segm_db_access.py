...
##----------------------------------------------------------------
    def get_image(self, im_file):
        ...
        list_of_imagettes=[]
        for para in ROI_pos: ##### change for a better imagettes' selection method___PREDICSTION--> Etienne's method
            outputs=[]
...
##----------------------------------------------------------------
    def get_im_and_segm(self, im_file, segm_dir, N_squares=16):
        ...
        [base_name, ext] = file_name.rsplit(".", 1)
        if 'Tumor' in im_file:
            cm =os.path.join(segm_dir, base_name + "_Mask" + "." + ext)
        else:
            cm = None
